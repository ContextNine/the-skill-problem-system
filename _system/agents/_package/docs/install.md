...
Run `./install.sh` from the fresh user-owned repository. The wizard creates managed runtime under the user's data directory, initializes private configuration under the user's config directory, installs the `fleet` launcher, and makes every public skill globally discoverable.
