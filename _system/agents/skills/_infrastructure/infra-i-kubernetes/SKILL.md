...
Use `$infra-i-code-folder-and-computer-topology` to resolve the current machine and the `k3s-infrastructure` checkout. Read its `AGENTS.md`, `README-infrastructure.md`, and the closest component README before acting.
...
- For database queries, locks, slots, WAL, or schema evidence, use `$infra-i-run-postgresql`.
...
- For application alerts and remediation orchestration, use the relevant repository skill, such as `$incident-controller-remediate-production-incidents`.
