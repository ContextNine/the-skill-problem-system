...
- OpenRouter uses its native Anthropic-compatible endpoint. Featherless exposes an OpenAI-compatible endpoint, so its launcher uses the already-approved CLIProxyAPI binary as an isolated loopback translator.
