...
- macOS and Linux: `_system/agents/_package/defaults/dependencies.json` owns `tmux`, `btop`, `starship`, and macOS `terminal-notifier`. Secondary Mac remains disabled; its historical prebuilt details are observation-only.
...
```bash
SKILL_DIR="$(vault root)/_system/agents/skills/_infrastructure/infra-i-manage-fleet-terminal-workspaces"
