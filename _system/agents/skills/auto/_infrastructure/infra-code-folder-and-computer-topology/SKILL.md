...
- Treat `skills/sources.json` repository-skill choices as an explicit allowlist consumed by agent sync; never infer projections for every registered repository. External checkouts derive below `<resolved Code root>/open_source`.
