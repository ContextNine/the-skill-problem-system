...
SKILL_ID = "infra-manage-rclone-google-drive"
...
def command_plan(desired: dict[str, Any], machine_id: str, executable: str) -> dict[str, Any]:
    ...
    actions = []
    if not installed:
        actions.append("preview and approve rclone installation through infra-update-fleet-dependencies")
    if missing:
        actions.append("complete and review non-secret desired Google Drive identifiers")
    actions.extend(
        [
            "enroll native OAuth client secret and generated config unlock through infra-onboard-machine",
