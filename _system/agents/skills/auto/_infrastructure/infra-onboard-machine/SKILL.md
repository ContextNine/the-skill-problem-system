...
5. Use `$infra-sync-code-workspaces` during onboarding for Code repositories and primary-owned Codex and Claude configuration. Do not recreate that sync here.
6. Use `$infra-manage-fleet-terminal-workspaces` for terminal packages, profiles, Warp, cmux, tmux, workmux, and terminal acceptance.
7. Use [[sops-key-enrollment-and-rotation|SOPS Key Enrollment and Rotation]] only when an owning repository requires shared SOPS access.
8. On Linux workers, install user-approved optional daemons or watchdogs only through [[linux-worker-onboarding-and-acceptance#Optional daemons and watchdogs|Linux Worker Optional Daemons And Watchdogs]]. Keep the registered set explicit, preview first, and verify the exact target after apply.
