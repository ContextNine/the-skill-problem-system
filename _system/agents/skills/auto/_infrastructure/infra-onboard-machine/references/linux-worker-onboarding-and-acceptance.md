...
- `$infra-code-folder-and-computer-topology` supplies current fleet and network facts.
...
- `$infra-sync-code-workspaces` owns Code repositories and personal Codex and Claude configuration.
- `$infra-manage-fleet-terminal-workspaces` owns terminal profiles and final Warp/cmux layout.
...
Install canonical provider launchers after native Claude Code on every active development machine. Configure and authenticate providers separately per [[README-claude-code-proxies|Claude Code Proxies]]:
...
```bash
for command_name in codex claude claude-codex claude-codex-high claude-codex-xhigh claude-kimi kimi opencode gh git git-lfs node npm t3 jq btop rg fd fzf curl wg bun uv gws gcloud rclone; do
...
```bash
ssh ctx9@BONJOUR_HOST 'sh -lc "command -v codex claude claude-codex claude-codex-high claude-codex-xhigh claude-kimi kimi opencode gh node t3 btop rg bun uv gws gcloud"'
```
...
```bash
secret_service_script="$(vault root)/_system/agents/skills/auto/_infrastructure/infra-onboard-machine/scripts/linux_secret_service.py"
...
Use `$infra-sync-code-workspaces` `migrate-github-remotes` after every selected machine passes authentication. For a manually reviewed exception, use canonical SSH and test fetch plus non-mutating push negotiation:
...
```bash
WORKSPACE_SKILL_DIR="$(vault root)/_system/agents/skills/auto/_infrastructure/infra-sync-code-workspaces"
...
set -eu
hostnamectl --static
for command_name in codex claude claude-codex claude-codex-high claude-codex-xhigh claude-kimi kimi opencode gh git node npm t3 jq btop rg curl bun uv gws gcloud; do command -v "$command_name"; done
codex login status
gh auth status --hostname github.com
...
```bash
watchdog_installer="$(vault root)/_system/agents/skills/auto/_infrastructure/infra-onboard-machine/scripts/linux_optional_watchdogs.py"
