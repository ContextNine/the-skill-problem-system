...
- **Development sync:** run `$infra-sync-code-workspaces` from the primary with the explicit onboarding target after authentication. The same apply owns repositories and personal Codex and Claude convergence.
...
- **Skills and terminal:** verify role-appropriate skill availability, then use `$infra-manage-fleet-terminal-workspaces` for terminal profiles and saved workspaces.
