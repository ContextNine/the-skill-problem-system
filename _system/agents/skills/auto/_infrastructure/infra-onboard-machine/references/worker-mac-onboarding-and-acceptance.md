...
This is the worker-Mac branch of `$infra-onboard-machine`. `$infra-manage-fleet-terminal-workspaces` owns Warp, cmux, terminal profiles, workmux, and T3 SSH terminal integration. `$infra-sync-code-workspaces` owns Code repositories and personal Codex and Claude configuration. Do not duplicate those procedures here.
...
```bash
python3 "$(vault root)/_system/agents/skills/auto/_infrastructure/infra-onboard-machine/scripts/bootstrap_worker_mac.py" finish MACHINE_ID
python3 "$(vault root)/_system/agents/skills/auto/_infrastructure/infra-onboard-machine/scripts/bootstrap_worker_mac.py" finish MACHINE_ID
```
...
```bash
python3 "$(vault root)/_system/agents/skills/auto/_infrastructure/infra-onboard-machine/scripts/bootstrap_worker_mac.py" verify MACHINE_ID --json
```
...
After acceptance, follow [[Vault Git Sync#Worktree coordination|Worktree coordination]]. The iCloud Vault is single-writer across Macs. A worker finishes file edits after filesystem durability and lease release without waiting for iCloud upload; the primary alone reviews, commits, and pushes changes visible in its worktree.
