...
```
  ...
  → $vibe-marketer-creative         Build this as a visual
                      landing page (~15 min)
  → "Skip to next"   Continue to $vibe-marketer-email-sequences
...
Rules:
- The visual conversion option ($vibe-marketer-creative) is always the FIRST next step
- The "skip" option explicitly names the next skill in the chain
- The user chooses -- the system does not auto-advance
- This applies to: $vibe-marketer-direct-response-copy, $vibe-marketer-lead-magnet, $vibe-marketer-newsletter,
...
Used by the orchestrator and `$vibe-marketer-start-here` to show the current state of the
user's project.
...
```
  ...
  │                                              │
  │  → $vibe-marketer-brand-voice    Build it now (~10 min)    │
...
point them to `$vibe-marketer-start-here` or suggest a new direction.
