---
name: vibe-marketer-seo-content
description: >
  Creates or refreshes publication-ready SEO articles from a target keyword,
  cluster, or content brief. Use when asked to write SEO content, create a blog
  post for a keyword, analyze current search results, integrate People Also Ask
  questions, add Article or FAQ schema, or update an existing article. It uses
  brand voice and keyword context, saves the content, and can chain to
  $vibe-marketer-content-atomizer for distribution.
---
...
4. If files are missing, proceed without them. Note at the end:
   ```
   → $vibe-marketer-brand-voice would let me match your exact tone
   → $vibe-marketer-keyword-research would provide a content brief
   → $vibe-marketer-positioning-angles would sharpen the angle
   ```
5. If no brand directory exists at all:
   ```
   No brand profile found — this skill works standalone.
   I'll ask what I need as we go. Run $vibe-marketer-start-here or
   $vibe-marketer-brand-voice later to unlock personalization.
   ```
...
If coming from $vibe-marketer-keyword-research skill, most of this is already defined in
...
competitor gaps. Show the RESEARCH MODE signal per `references/brand-memory.md`
...
$vibe-marketer-keyword-research (at `./campaigns/content-plan/{keyword-slug}.md`), load it
...
formatting from `references/output-format.md`.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  → $vibe-marketer-creative           Build it — featured image,
  ...
  → $vibe-marketer-content-atomizer   Distribute across social —
                        LinkedIn, Twitter, Instagram,
                        TikTok, and more (~10 min)
  → $vibe-marketer-newsletter         Feature this article in your
                        next newsletter edition (~15 min)
  → $vibe-marketer-email-sequences    Nurture readers who find this
                        into subscribers (~15 min)
  → $vibe-marketer-seo-content        Write the next article from
...
## Chain to $vibe-marketer-content-atomizer (v2 Enhancement)
...
```
  ...
  → "Atomize" to run $vibe-marketer-content-atomizer now
...
If the user says "atomize" or similar, hand off to $vibe-marketer-content-atomizer with:
...
### Input from $vibe-marketer-keyword-research skill:
...
**The flow:**
1. $vibe-marketer-keyword-research identifies the opportunity and creates content briefs
2. $vibe-marketer-positioning-angles finds the unique angle
3. $vibe-marketer-brand-voice defines how it should sound
4. **$vibe-marketer-seo-content creates the actual piece** (you are here)
5. $vibe-marketer-content-atomizer distributes it across channels
6. $vibe-marketer-direct-response-copy punches up CTAs
...
This is the full decision tree for every $vibe-marketer-seo-content invocation:

```
  $vibe-marketer-seo-content invoked
  |
  ...
  |       |
  |       +-- Phase 2: Content Brief
  |       |   +-- Load from $vibe-marketer-keyword-research or create new
  |       |
  |       +-- Phase 3: Outline
  ...
  |   +-- FILES SAVED
  |   +-- WHAT'S NEXT (with $vibe-marketer-content-atomizer chain)
  |
  +-- Chain offer: Atomize into social with $vibe-marketer-content-atomizer?
  |
  +-- Feedback Collection
...
```
  ...
  |  -> $vibe-marketer-keyword-research to find the right one  |
...
```
  ...
  |                                              |
  |  -> $vibe-marketer-brand-voice  Build your profile (~10 min|
...
```
  ...
  $vibe-marketer-seo-content again and tell me.)
```
...
- Log to ./brand/learnings.md under "What Works":
  `- [{date}] [$vibe-marketer-seo-content] Article "{title}" shipped as-is. Keyword: "{keyword}". Angle: {angle}. Word count: {N}. Content type: {type}.`
...
- Example entry: `- [{date}] [$vibe-marketer-seo-content] User softened tone in intro. Note: default opening may be too aggressive for this audience.`
...
- If the rewrite reveals a pattern (e.g., voice is consistently wrong),
  suggest re-running $vibe-marketer-brand-voice.
- Example entry: `- [{date}] [$vibe-marketer-seo-content] User rewrote "{title}" -- shifted from data-driven to story-driven. Voice profile may need update.`
...
8. **Always offer the $vibe-marketer-content-atomizer chain.** After creating an article,
...
9. **Write the content brief if one does not exist.** If the user provides
   a keyword but no content brief exists from $vibe-marketer-keyword-research, create one
...
10. **Respect the file output format exactly.** The frontmatter schema is
    ...
    Get these right so other skills (especially $vibe-marketer-content-atomizer) can find
...
14. **Register every content piece.** After saving the article, append an
    ...
    This keeps the asset registry current for $vibe-marketer-start-here and other skills.
