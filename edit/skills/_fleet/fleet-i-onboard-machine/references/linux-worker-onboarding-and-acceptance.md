...
- `$fleet-i-code-folder-and-computer-topology` supplies current fleet and network facts.
...
- `$fleet-i-sync-code-workspaces` owns Code repositories and personal Codex and Claude configuration.
- `$fleet-i-manage-terminal-workspaces` owns terminal profiles and final Warp/cmux layout.
...
```bash
secret_service_script="$(vault root)/_system/agents/edit/skills/_fleet/fleet-i-onboard-machine/scripts/linux_secret_service.py"
...
Use `$fleet-i-sync-code-workspaces` `migrate-github-remotes` after every selected machine passes authentication. For a manually reviewed exception, use canonical SSH and test fetch plus non-mutating push negotiation:
...
```bash
WORKSPACE_SKILL_DIR="$(vault root)/_system/agents/edit/skills/_fleet/fleet-i-sync-code-workspaces"
...
```bash
watchdog_installer="$(vault root)/_system/agents/edit/skills/_fleet/fleet-i-onboard-machine/scripts/linux_optional_watchdogs.py"
