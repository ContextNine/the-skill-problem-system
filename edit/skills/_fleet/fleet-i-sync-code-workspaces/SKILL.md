...
Before repository work, the source and target workers inspect `<Code root>/.codefoldersync/config.json`. A structurally valid V3 projection in `adoption` leaves workspace sync authoritative. A valid `normal` projection makes every workspace operation report-only, including an invocation that requested `--apply`. A partial, unsafe, malformed, or root-mismatched control directory blocks repository mutation. The handoff never writes `.workspace-sync` state.
