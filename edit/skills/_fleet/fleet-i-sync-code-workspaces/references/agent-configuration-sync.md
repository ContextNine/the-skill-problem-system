...
| Repositories and personal agent configuration convergence | `$fleet-i-sync-code-workspaces` |
| New-machine gate ordering | `$fleet-i-onboard-machine` |
| Worker-Mac applications, GUI consent, and unattended-operation checks | `$fleet-i-onboard-machine` worker-Mac route |
| tmux, workmux, Warp, cmux, and terminal profiles | `$fleet-i-manage-terminal-workspaces` |
...
```bash
SKILL_DIR="$(vault root)/_system/agents/edit/skills/_fleet/fleet-i-sync-code-workspaces"
