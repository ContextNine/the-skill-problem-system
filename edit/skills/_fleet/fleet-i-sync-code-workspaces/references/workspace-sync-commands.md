...
Every command checks CodeFolderSync ownership before repository preflight or state creation. Once a structurally valid V3 projection for the exact Code root reaches lifecycle `normal`, an `--apply` request returns `codefoldersync-report-only`. It may inspect local Git state and compare remote heads, but it cannot fetch, fast-forward, clone, move, rewrite a remote, adopt catalog paths, or write `.workspace-sync` history. Lifecycle `adoption` retains the behavior above. An invalid `.codefoldersync` control directory blocks instead of falling back to mutation.
