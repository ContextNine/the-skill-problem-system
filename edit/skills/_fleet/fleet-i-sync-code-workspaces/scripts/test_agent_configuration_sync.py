            ...
            "machines": [
                {
                    ...
                    "role": "primary",
                    "platform": "macos",
                    "template_variants": ["macos", "primarymac"],
                ...
                },
                {
                    ...
                    "role": "worker",
                    "platform": "macos",
                    "template_variants": ["macos", "workermac"],
                    "roots": {"code": "~/code", "vault": "~/Vault"},
...
''',
        ...
        (config / "skills/skill-sources.json").write_text(
            json.dumps({
                "schema_version": 4,
                "gh_skills": {},
                "repos": [],
...
''',
            ...
            self.assertEqual(
                json.loads((target / ".agents/settings/integrations/langfuse.json").read_text()),
                {
                    "schema_version": 2,
                    "instance_id": "test",
                    "enabled": True,
    ...
    def test_role_rendering_uses_registry_addresses_and_linux_novnc_guidance(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertIn("10.13.13.2", rendered["primary"])
            self.assertIn("worker-mac.example.ts.net", rendered["worker-mac"])
            self.assertIn("10.13.13.10", rendered["linux-worker"])
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(
                config_paths,
                {
                    ...
                    ".agents/settings/integrations/langfuse.json",
                    ".agents/settings/skills/skill-sources.json",
                },
            )
            self.assertEqual(
                instruction_paths,
                {".agents/instructions/AGENTS.md", ".codex/AGENTS.md", ".claude/CLAUDE.md"},
            )
    ...
    def test_config_sync_installs_primary_non_secret_metadata(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            metadata = source / ".agents/settings/integrations/langfuse.json"
    ...
    def test_config_sync_replaces_stale_bootstrap_workspace_registry(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            source, registry = self.create_source(root)
            desired = root / "_system/agents/edit/settings/fleet/workspaces.json"
