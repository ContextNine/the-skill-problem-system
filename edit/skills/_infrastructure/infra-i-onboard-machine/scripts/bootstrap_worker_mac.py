...
SCRIPT_RELATIVE = Path(
    "_system/agents/edit/skills/_infrastructure/"
    "infra-i-onboard-machine/scripts/bootstrap_worker_mac.py"
)
...
def command_seed(root: Path, machine_id: str, *, apply: bool) -> int:
    ...
    if not auth.get("github_ssh"):
        steps.append(
            "From Primary machine, run the infra-i-onboard-machine GitHub fleet-auth provision and enrollment workflow for this machine; key enrollment is a separate human approval checkpoint."
        )
