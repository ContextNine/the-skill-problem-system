...
Never shorten the token-bearing URL. Promotion URLs may use `$marketing-manage-utm-tracking-links` when explicitly requested. Use `$infra-i-file-upload` for the actual asset, then store its returned public URL as `downloadUrl` on the destination page/config.
