...
`fleet source path` prints the editable repository. `fleet config path` prints the installed settings under `~/.agents/`. Installing a Vault later connects its path to the existing editable source; it does not create a second one.
