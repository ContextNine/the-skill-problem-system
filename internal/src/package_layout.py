...
AGENTS_ROOT = INTERNAL_ROOT.parent
SYSTEM_ROOT = AGENTS_ROOT.parent
VAULT_ROOT = SYSTEM_ROOT.parent

EDIT_ROOT = AGENTS_ROOT / "edit"
SKILLS_ROOT = EDIT_ROOT / "skills"
...
OVERLAY_SKILLS_ROOT = GENERATED_ROOT / "overlays"
SNAPSHOT_SKILLS_ROOT = GENERATED_ROOT / "snapshots"
CATALOG_ROOT = GENERATED_ROOT / "catalog"
...
INSTRUCTION_FRAGMENTS_PATH = INTERNAL_ROOT / "instructions/render.json"
...
def resolve_agents_root(root: Path) -> Path:
    """Accept either the standalone package root or the containing Vault root."""
    root = root.expanduser().resolve()
    if (root / "edit").is_dir() and (root / "internal").is_dir():
        return root
    candidate = root / "_system/agents"
    if (candidate / "edit").is_dir() and (candidate / "internal").is_dir():
        return candidate
    raise ConfigurationError(f"agent source root is missing edit/ and internal/: {root}")


def installed_package_root(home: Path | None = None) -> Path:
    return (home or Path.home()) / ".agents/internal"


def installed_config_root(home: Path | None = None) -> Path:
    return (home or Path.home()) / ".agents/settings"


def installed_state_root(home: Path | None = None) -> Path:
    return (home or Path.home()) / ".agents/state"
