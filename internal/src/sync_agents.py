...
def validate_machine_secrets(root: Path) -> None:
    ...
    registry = load_json_yaml(
        agents / "edit/settings/fleet/machine-secrets.json", "machine secret registry"
    )
...
def invoke_workspace_sync(
    ...
    *,
    ...
    command = [
        ...
        "reconcile",
        "--catalog",
        str(agents / "edit/settings/fleet/workspaces.json"),
        "--machine-registry",
        str(agents / "edit/settings/fleet/machines.json"),
...
def write_aggregate_lock(
    ...
    *,
    ...
    path = resolve_agents_root(root) / "internal/generated/state/dependencies.lock.json"
    try:
...
def build_skill_snapshot_bundle(
    ...
    *,
    ...
    template_values = (
        global_agent_configuration.machine_template_values(
            registry,
            machine,
            source_home=source_home or Path.home(),
        )
        if registry is not None and machine is not None
        else None
    )
...
def invoke_skill_targets(
    machines: list[dict[str, Any]],
    bundles: dict[str, skill_snapshots.SnapshotBundle],
    *,
    apply: bool,
    verify: bool,
    backup_suffix: str,
...
def sync(args: argparse.Namespace) -> int:
    ...
    registry_path = args.machine_registry or (
        agents / "edit/settings/fleet/machines.json"
    )
    ...
    if "dependencies" in parts:
        manifest_path = args.dependency_manifest or (
            agents / "internal/defaults/dependencies.json"
        )
    ...
    if "workspace-deps" in parts:
        workspace_dependency_manifest = load_json_yaml(
            agents / "edit/settings/dependencies/selections.json", "workspace dependency manifest"
        )
        workspaces_manifest = load_json_yaml(
            agents / "edit/settings/fleet/workspaces.json", "workspace manifest"
        )
        direct_for_validation = dependency_manifest or load_json_yaml(
            agents / "internal/defaults/dependencies.json", "agent dependency registry"
        )
    ...
    skill_bundles: dict[str, skill_snapshots.SnapshotBundle] = {}
    ...
    if "skills" in parts:
        ...
        skill_bundles = {
            str(selected["id"]): build_skill_snapshot_bundle(
                root,
                require_repo_sources=args.require_repo_sources,
                registry=registry,
                machine=selected,
            ...
            for selected in selected_machines
        }
        local_bundle = skill_bundles[source_id]
