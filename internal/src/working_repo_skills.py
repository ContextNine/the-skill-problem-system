...
SELECTION = Path("edit/settings/skills/skill-sources.json")
GH_ROOT = Path("edit/skills/github")
OVERLAY_ROOT = Path("internal/generated/overlays")
SNAPSHOT_ROOT = Path("internal/generated/snapshots")
...
def overlay_is_current(target: Path, source: Path, marker: str) -> bool:
    ...
    actual = {
        ...
        if child.name != MARKER and not ICLOUD_DUPLICATE_RE.fullmatch(child.name)
    }
    if expected != actual:
        return False
